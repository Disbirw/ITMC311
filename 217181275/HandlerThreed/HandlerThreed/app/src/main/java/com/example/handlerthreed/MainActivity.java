...
//testing thread’s termination
            ...
            if(bar1.getProgress() == bar1.getMax()){
                ...
                //إذا امتلأ شريط التقدم، يتم إخفاؤه وتحديث النص.

            else{
                msgWorking.setText("Working..."+
                        bar1.getProgress() );
            }
                //وإلا، يعرض عدد الثواني الحالية
        }
    }; //handler
    ...
    public void onCreate(Bundle icicle) {
        ...
        // ربط عناصر الواجهة بالمتغيرات في الكود

        strTest+= "-01"; // slightly change the global string
        intTest= 1;      // تعديل طفيف على المتغيرات المشتركة.
    }//onCreate
    public void onStop() {
        super.onStop();
        isRunning= false;
    }    // يمنع استمرار الخيط عند الخروج من النشاط.
...
// bar1.setProgress(0);
        Thread background = new Thread(new Runnable() {
            public void run() {
                // إنشاء خيط جديد يعمل في الخلفية
                try{
...
//we can see and change (global) class variables
                        data += "\n"+ strTest+ " "+ intTest;
                        intTest++;
                                  //يتم إنشاء قيمة عشوائية وإرفاقها مع المتغيرات المشتركة
